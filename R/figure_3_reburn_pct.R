...
modis_fires <- st_read("data/modis/modis_event_polygons_cus.gpkg") %>%
  filter(id == 25211 | id == 29790) %>%
  mutate(id = as.character(id))

mtbs_fires <- mtbs_fire%>%
...
ignitions <- st_read(file.path("data/modis/modis_event_polygons_cus.gpkg")) %>%
...
mtbs_labs <- mtbs_fires %>%
  st_set_geometry(NULL) %>%
  dplyr::select(fire_name, discovery_year, discovery_month, discovery_day) %>%
  mutate(label = paste0("Start: ", discovery_year,"-", discovery_month,"-", discovery_day),
         X = c(-8055000, -8055000),
         Y = c(2995000, 2997000))
...
ggarrange(p1, p2, ncol = 2, nrow=1) +
  ggsave("results/draft_figures/figure_3_reburn_fig.png", dpi = 300, width = 10, height = 5) +
  ggsave("results/draft_figures/figure_3_reburn_fig.pdf", dpi = 600, width = 10, height = 5) 
